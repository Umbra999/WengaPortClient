...
namespace WengaPort.Modules
{
    class OnGui
    {
        ...
        {
			bool flag = RoomManager.field_Internal_Static_ApiWorldInstance_0 == null;
			if (flag)
			{
			...
			else
            {
				try
				{
					...
					GUI.Label(new Rect(Screen.width - 130f, 10f, 500f, 500f), string.Concat(new object[]
					{
						...
						string.Format("\n<b>USpeak: {0}</b>", Utils.CurrentUser.prop_USpeaker_0.CurrentBitrate),
						string.Format("\n<b>Sending: {0}</b>", Utils.CurrentUser.prop_USpeaker_0.SendingMode),
